  ...
  return (
    <div
      className={`card group relative flex h-full flex-col overflow-hidden transition duration-300 motion-reduce:transition-none ${
        selected
          ? "border-gold ring-2 ring-gold/35 shadow-[0_18px_50px_rgba(184,137,61,0.18)]"
          : ""
      }`}
    >
      <button
        ...
        <div className="relative aspect-[16/10] w-full overflow-hidden bg-bg-elevated">
          <SmartImage
            ...
            className={`transition duration-500 motion-reduce:transition-none ${
              hover ? "group-hover:opacity-0" : "group-hover:scale-[1.035]"
            }`}
            priority={priority}
          />
          {hover ? (
            <SmartImage
              ...
              fill
              sizes="(max-width: 640px) 100vw, (max-width: 1200px) 50vw, 33vw"
              className="opacity-0 transition duration-500 group-hover:opacity-100 motion-reduce:transition-none"
            />
          ) : null}
          ...
          <div className="absolute left-3 top-3 z-10 flex flex-wrap gap-2">
            <span className="rounded-full bg-white/92 px-2.5 py-1 text-[0.7rem] font-medium text-gold-deep shadow-sm backdrop-blur">
              {plan.style}
            </span>
          ...
          <div className="absolute right-3 top-3 z-10 rounded-full bg-white/92 px-3 py-1 text-[0.7rem] font-semibold text-ivory shadow-sm backdrop-blur">
      ...
      <div className="mt-auto flex items-center justify-between gap-3 border-t border-border px-5 py-3">
        ...
        {media?.floorPlan ? (
          <span className="text-xs uppercase tracking-[0.12em] text-text-dim">
